...
class BeamSearchDecoder(TokenDecoder):
    ...
    def update(
        ...
        logprobs = (
            ...
            - mx.logsumexp(logits.astype(mx.float32), axis=-1, keepdims=True)
        )
    ...
    def finalize(self, tokens: mx.array, sum_logprobs: mx.array):
        ...
        tokens_out = [
            [list(seq) for seq in sequences.keys()] for sequences in self.finished_sequences
        ]
        sum_out = [
            [float(score) for score in sequences.values()] for sequences in self.finished_sequences
        ]
    ...
    def run(self, mel: mx.array) -> List[DecodingResult]:
        ...
        if self.n_group > 1:
            ...
            tokens = tokens.reshape(
                (n_audio * self.n_group, len(self.initial_tokens))
            )
        ...
        tokens = [
            [
                t[: t.index(tokenizer.eot) if tokenizer.eot in t else len(t)]
                for t in s
            ]
            for s in tokens
        ]
