...
class LightningWhisperMLX():
    ...
    def transcribe(self, audio_path, language=None, **decode_options):
        # print(f"Transcribing {audio_path} with {self.name} and {decode_options}")
        result = transcribe_audio(audio_path, path_or_hf_repo=f'./mlx_models/{self.name}', language=language, batch_size=self.batch_size, **decode_options)
        return result