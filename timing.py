...
def median_filter(x: np.ndarray, filter_width: int):
    ...
    # Create a kernel for medfilt that only filters along the last dimension
    kernel_size = [1] * x.ndim
    kernel_size[-1] = filter_width

    # scipy.signal.medfilt handles padding with mode='reflect' by default
    result = signal.medfilt(x.astype(np.float32), kernel_size=tuple(kernel_size))
